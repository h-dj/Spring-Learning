...
import cn.reid.springbatchdemo.mapper.EnrollmentFieldSetMapper;
...
import cn.reid.springbatchdemo.processor.EnrollmentProcessor;
...
public class EnrollmentStepConfig {
    ...
    @Bean
    @StepScope
    public FlatFileItemReader<Enrollment> enrollmentReader(
            @Value("#{jobParameters['filePath']}") String filePath) {

        FlatFileItemReader<Enrollment> reader = new FlatFileItemReader<>();
        reader.setResource(new FileSystemResource(filePath));
        reader.setLinesToSkip(1);

        DefaultLineMapper<Enrollment> lineMapper = new DefaultLineMapper<>();
        ...
        EnrollmentFieldSetMapper fieldSetMapper = new EnrollmentFieldSetMapper();
    ...
    public JdbcBatchItemWriter<Enrollment> enrollmentWriter(
        ...
        JdbcBatchItemWriter<Enrollment> writer = new JdbcBatchItemWriter<>();
    ...
    @Bean
    public Step enrollmentStep(
            JobRepository jobRepository,
            PlatformTransactionManager transactionManager,
            FlatFileItemReader<Enrollment> enrollmentReader,
            EnrollmentProcessor enrollmentProcessor,
            JdbcBatchItemWriter<Enrollment> enrollmentWriter,
        ...
        return new StepBuilder("enrollmentStep", jobRepository)
                .<Enrollment, Enrollment>chunk(500, transactionManager)
