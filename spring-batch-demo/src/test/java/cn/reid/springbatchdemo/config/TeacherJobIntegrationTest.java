...
@SpringBatchTest
@SpringBootTest
class TeacherJobIntegrationTest {
    ...
    @Test
    void shouldProcessTeacherFileCorrectly() throws Exception {
        // 准备测试参数
        JobParameters params = new JobParametersBuilder()
                .addString("fileType", "teacher")
                .addString("filePath", "src/test/resources/data/teacher-test.dat")
        ...
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
                "SELECT * FROM t_teacher ORDER BY teacher_no");

        assertEquals(3, rows.size());

        // 验证第一条数据
        Map<String, Object> row1 = rows.get(0);
        assertEquals("T001", row1.get("teacher_no"));
        assertEquals("ACTIVE", row1.get("status"));

        // 验证第二条数据的 status 已被转为大写
        Map<String, Object> row2 = rows.get(1);
        assertEquals("T002", row2.get("teacher_no"));
        assertEquals("ACTIVE", row2.get("status"));

        // 验证第三条数据的 title 已被转为大写
        Map<String, Object> row3 = rows.get(2);
        assertEquals("T003", row3.get("teacher_no"));
        assertEquals("LECTURER", row3.get("title"));
    }
}
