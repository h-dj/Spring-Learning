...
@SpringBatchTest
@SpringBootTest
class CourseJobIntegrationTest {
    ...
    @Test
    void shouldProcessCourseFileCorrectly() throws Exception {
        // 准备测试参数
        JobParameters params = new JobParametersBuilder()
                .addString("fileType", "course")
                .addString("filePath", "src/test/resources/data/course-test.dat")
        ...
        List<Map<String, Object>> rows = jdbcTemplate.queryForList(
                "SELECT * FROM t_course ORDER BY course_code");

        assertEquals(3, rows.size());

        // 验证第一条数据
        Map<String, Object> row1 = rows.get(0);
        assertEquals("CS001", row1.get("course_code"));
        assertEquals("REQUIRED", row1.get("course_type"));
        assertEquals("ACTIVE", row1.get("status"));

        // 验证第二条数据的 status 已被转为大写
        Map<String, Object> row2 = rows.get(1);
        assertEquals("CS002", row2.get("course_code"));
        assertEquals("ELECTIVE", row2.get("course_type"));
        assertEquals("ACTIVE", row2.get("status"));

        // 验证第三条数据的 status 已被转为大写
        Map<String, Object> row3 = rows.get(2);
        assertEquals("CS003", row3.get("course_code"));
        assertEquals("PUBLIC", row3.get("course_type"));
